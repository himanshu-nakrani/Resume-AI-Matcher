...
import { Bookmark, ChevronDown, Search, SlidersHorizontal, ExternalLink } from "lucide-react";
...
import { getHostname, cleanJobText, jobSnippets, jobBadges, formatPublishedDate } from "./helpers";
...
export function JobSearchSection({
  ...
  const activeFilterCount = [exaExperienceLevel, exaJobType, exaRemote, exaSalaryMin, exaIndustry, exaCompanySize].filter(Boolean).length;
      ...
      <details className="mt-10 group">
        ...
        <div className="mt-3 space-y-4">
          ...
          <Button type="button" variant="ghost" size="sm" onClick={() => setShowFilters(!showFilters)}>
            <SlidersHorizontal className="w-3.5 h-3.5 mr-1.5" /> Filters {activeFilterCount > 0 ? `(${activeFilterCount})` : ""}
          </Button>
          ...
          <div className="flex flex-wrap gap-3 items-end">
            ...
            <Button type="button" variant="secondary" onClick={onSearch} disabled={isSearchPending}>
                    ...
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                      ...
                      <div className="flex items-center gap-1 shrink-0" onClick={(e) => e.stopPropagation()}>
                        {!matchInfo && hasResumeText && (
                          <Button type="button" size="sm" variant="ghost" className="text-[11px]" onClick={() => onPreScreen(hit)}>Match</Button>
                        )}
                        {matchInfo?.loading && <span className="text-[11px] text-muted-foreground px-2">...</span>}
                        <Button type="button" size="sm" variant="ghost" onClick={() => onSaveJob(hit)} title="Save job"><Bookmark className="w-3.5 h-3.5" /></Button>
                        <Button type="button" size="sm" variant="outline" onClick={() => onImportJd(hit.url)} disabled={isFetchJobPending}>
